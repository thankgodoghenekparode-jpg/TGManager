...
React 19 + Vite + TypeScript frontend for the TGManager company operation
management system. Consumes the NestJS backend at
your TGManager API (or a local instance).
...
- **Deployed backend**: copy `.env.example` to `.env` and set
  `VITE_API_BASE_URL=https://your-tgmanager-api.example.com/api/v1`.

## Auth & multi-tenancy

- Session auth uses HTTP-only cookies (`tgmanager_access` / `tgmanager_refresh`);
