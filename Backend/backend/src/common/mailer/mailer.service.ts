...
export class MailerService {
  ...
  async smtpConfig(): Promise<SmtpConfig> {
    ...
    const from =
      ...
      'Zarox Connect <no-reply@zarox.example>';
  ...
  async sendPasswordReset(to: string, resetUrl: string): Promise<boolean> {
    return this.send({
      to,
      subject: 'Reset your Zarox Connect password',
      text:
        'A password reset was requested for your Zarox Connect account. Open ' +
        'the link below to choose a new password:\n\n' +
        `${resetUrl}\n\n` +
        'If you did not request this, you can safely ignore this email.',
      html:
        '<p>A password reset was requested for your Zarox Connect account. ' +
