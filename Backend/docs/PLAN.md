# TGManager - Development Plan

Implementation roadmap for **tgmanager-api** (NestJS + Prisma + PostgreSQL).
...
- **Chat**
  - Socket.io gateway with session auth (JWT via `auth.token` or `tgmanager_access` cookie) + tenant-membership verification; joins `user:{id}` and `tenant:{id}` rooms; rejects and disconnects unauthorized clients.
...
- Tests: `platform.e2e-spec.ts` (20 cases: access control, plan CRUD/limits, tenant admin, suspension, enforcement matrix), `plan-limits.service.spec.ts` unit tests; CI validated locally against `tgmanager_test`.
